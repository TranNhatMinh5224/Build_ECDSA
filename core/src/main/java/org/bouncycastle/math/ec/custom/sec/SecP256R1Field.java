...
public class SecP256R1Field
{
    ...
    static final int[] P = new int[]{ 0x51BAFF0F, 0x5213A3E4, 0xCA0EF78B, 0x61788699, 0x53A69F2A, 0x602D68B5,
        0xFFE6CCAE, 0xBD360F61 };

    static final BigInteger p_ = fromHex("BD360F61FFE6CCAE602D68B553A69F2A61788699CA0EF78B5213A3E451BAFF0F");
    ...
    public static void add(int[] x, int[] y, int[] z)
    {
        ...
        //     addPInvTo(z);
        // }
        BigInteger x_ = Nat256.toBigInteger(x);
        BigInteger y_ = Nat256.toBigInteger(y);
        BigInteger m = (x_.add(y_)).mod(p_);
        int[] r = Nat256.fromBigInteger(m);
    ...
    public static void multiply(int[] x, int[] y, int[] z)
    {
        ...
        BigInteger x_ = Nat256.toBigInteger(x);
        BigInteger y_ = Nat256.toBigInteger(y);
        BigInteger m = (x_.multiply(y_)).mod(p_);
        int[] r = Nat256.fromBigInteger(m);
        for (int i = 0; i < z.length; i++)
        {
            z[i] = r[i];
...
//          assert cc == 0 || cc == 1;
    ...
    public static void square(int[] x, int[] z)
    {
        ...
        // reduce(tt, z);
        BigInteger x_ = Nat256.toBigInteger(x);
        BigInteger m = (x_.multiply(x_)).mod(p_);
        int[] r = Nat256.fromBigInteger(m);
        for (int i = 0; i < z.length; i++)
        {
            z[i] = r[i];
...
//        assert n > 0;
    ...
    public static void subtract(int[] x, int[] y, int[] z)
    {
        BigInteger x_ = Nat256.toBigInteger(x);
        BigInteger y_ = Nat256.toBigInteger(y);
        BigInteger m = ((x_.add(p_)) .subtract(y_)).mod(p_);
        int[] r = Nat256.fromBigInteger(m);
        for (int i = 0; i < z.length; i++)
        {
            z[i] = r[i];
