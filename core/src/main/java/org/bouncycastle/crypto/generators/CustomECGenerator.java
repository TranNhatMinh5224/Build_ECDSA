...
public class CustomECGenerator
{
    ...
    public static BigInteger[] generateSafePrime(int L, int N, SecureRandom random, int certainty)
    {
        ...
        if (L < 2 * (N + 1))
        {
            throw new IllegalArgumentException("L phai >= 2(N+1)");
        }
        ...
        if (q == null)
        {
            throw new IllegalStateException("FATAL: Khong tim duoc q trong khoang N-bit");
        }
        ...
        for (int i = 1; i < k; i++)
        {
            ...
            if (qi == null)
            {
                throw new IllegalStateException("Khong tim duoc qi tai buoc i=" + i);
            }
        ...
        if (A.compareTo(B) > 0)
        {
            throw new IllegalStateException("Khoang [A, B] rong.");
        }
        ...
        if (!startQk.testBit(0))
        {
            startQk = startQk.add(ONE);
        }
            ...
            if (qk.isProbablePrime(certainty))
            {
                ...
                if (p.bitLength() == L && p.isProbablePrime(certainty))
                {
                    if (qk.compareTo(q) >= 0)
                    {
                        ...
                        if (!pMinus1.mod(f).equals(ZERO))
                        {
                            throw new IllegalStateException("Check Fail: f not divide p-1");
                        }
                        if (!pMinus1.mod(q).equals(ZERO))
                        {
                            throw new IllegalStateException("Check Fail: q not divide p-1");
                        }
