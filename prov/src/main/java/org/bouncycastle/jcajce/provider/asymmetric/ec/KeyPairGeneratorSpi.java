    ...
    extends java.security.KeyPairGenerator
{
        ...
        extends KeyPairGeneratorSpi
    {
        ...
        {
            System.out.println("[KeyPairGeneratorSpi.generatedKeyPair]");
            if (!initialised)
            {
            ...
            if (ecParams instanceof ECParameterSpec)
            {
                ...
                BCECPrivateKey privKey = new BCECPrivateKey(algorithm, priv, pubKey, p, configuration);

                byte[] privateKeyInfo = privKey.getEncoded();
                byte[] privateKeySecret = privKey.getD().toByteArray();
                System.out.println("[CASE 1]");
                System.out.printf("[private key secret] bytes: %d, hex: %s\n",privateKeySecret.length, Hex.toHexString(privateKeySecret));
                System.out.printf("[private key info  ] bytes: %d, hex: %s\n", privateKeyInfo.length, Hex.toHexString(privateKeyInfo));

                byte[] publicKeyInfo = pubKey.getEncoded();
                System.out.printf("[public key info   ] bytes: %d, hex: %s\n", publicKeyInfo.length, Hex.toHexString(publicKeyInfo));
                return new KeyPair(pubKey, privKey);
            }
            else if (ecParams == null)
            {
                BCECPublicKey pubKey = new BCECPublicKey(algorithm, pub, configuration);
                BCECPrivateKey privKey = new BCECPrivateKey(algorithm, priv, configuration);
            ...
            else
            {
                ...
                BCECPublicKey pubKey = new BCECPublicKey(algorithm, pub, p, configuration);
                BCECPrivateKey priKey = new BCECPrivateKey(algorithm, priv, pubKey, p, configuration);

                byte[] privateKeyInfo = priKey.getEncoded();
                byte[] privateKeySecret = priKey.getD().toByteArray();
